...
{
	HAL_SPI_Transmit(&oled_spi_hander, OLED_BUFFER, sizeof(OLED_BUFFER), 500);
}
...
/**
  ...
  * @retval None
  */
void oled_draw_pixel(uint16_t Xpos, uint16_t Ypos)
{
...
void oled_draw_hline(uint16_t Xpos, uint16_t Ypos, uint16_t Length)
{
  uint32_t index = 0;
	...
	{
		oled_draw_pixel((Xpos + index), Ypos);
	}
}
...
void oled_draw_vline(uint16_t Xpos, uint16_t Ypos, uint16_t Length)
{
  uint32_t index = 0;
	...
	for(index = 0; index < Length; index++)
	{
		oled_draw_pixel(Xpos, Ypos + index);
	}
}
...
void oled_draw_line(uint16_t X1, uint16_t Y1, uint16_t X2, uint16_t Y2)
{
  ...
  {
    oled_draw_pixel(x, y);  /* Draw the current pixel */
...
void oled_draw_circle(uint16_t Xpos, uint16_t Ypos, uint16_t Radius)
{
  ...
  while (CurX <= CurY)
  {
    oled_draw_pixel((Xpos + CurX), (Ypos - CurY));

    oled_draw_pixel((Xpos - CurX), (Ypos - CurY));

    oled_draw_pixel((Xpos + CurY), (Ypos - CurX));
...
void oled_draw_ellipse(int Xpos, int Ypos, int XRadius, int YRadius)
{
  ...
  do {      
    oled_draw_pixel((Xpos-(uint16_t)(x/K)), (Ypos+y));
    oled_draw_pixel((Xpos+(uint16_t)(x/K)), (Ypos+y));
    oled_draw_pixel((Xpos+(uint16_t)(x/K)), (Ypos-y));
    oled_draw_pixel((Xpos-(uint16_t)(x/K)), (Ypos-y));      
